...
sched_print_tasks(void) {
  ...
  printf("Process INFO: ID: %d | PID: %d | Name:%s -- !Present Process!\n", dummy->id, dummy->PID, dummy->name);

  while(dummy->next != present_proc)
  {
      dummy = dummy->next;
      printf("Process INFO: ID: %d | PID: %d | Name: %s\n", dummy->id, dummy->PID, dummy->name);
  }
}
...
sigchld_handler(int signum) {
  ...
  for(;;)
  {
    ...
    if (WIFEXITED(status)  || WIFSIGNALED(status))
    {
      ...
      {
        /*
         * In the case where no more processes
         * exist, then move on to the following message!
         */
